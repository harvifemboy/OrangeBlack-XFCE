...
The GTK3 and GTK4 styles explicitly target Xfce panel tasklist buttons and their labels so open applications remain readable on a black panel and the active application is shown with orange background and black text.

The GTK3 and GTK4 styles leave the Xfce Screenshooter region-selection dialog surface transparent by separating the generic dialog background from the special `.dialog-vbox` used during region capture. This prevents the theme's black dialog background from becoming an opaque full-screen block. Xfce has also documented a separate compositor-related black-screen bug, so disabling the Xfwm4 compositor is still a useful fallback if the issue persists on a particular Xfce build. citeturn660110search1turn660110search3

After installing an updated copy, restart the Xfce panel with `xfce4-panel -r` or run the installer again.
